...
// Audit-only: reads build artifacts and makes serial GET requests to the
// temporary local production preview. It never calls scan APIs or providers.
...
for (const route of routes) {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const response = await fetch('http://127.0.0.1:3015' + route);
...
fs.writeFileSync('output/full-audit-2026-09-05/metrics.json', JSON.stringify(result, null, 2) + '\n');
